...
class DiscretePDF {
 public:
  ...
  size_t Sample(Float sample_value) const {
  ...
  size_t Sample(Float sample_value, Float &pdf) const {
  ...
  size_t SampleReuse(Float &sample_value) const {
    size_t index = Sample(sample_value);
    sample_value = (sample_value - cdf_[index]) / (cdf_[index + 1] - cdf_[index]);
    return index;
  }

  size_t SampleReuse(Float &sample_value, Float &pdf) const {
